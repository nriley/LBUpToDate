...
def archive_bundle(bundle_path, version):
    ...
    archive_path = os.path.join(dir_path, '%s-%s%s' % (bundle_name, version, bundle_ext))
    subprocess.check_call(['/usr/bin/ditto', '--keepParent', '-ck', bundle_path, archive_path])
...
def release(version, github_access_token):
    ...
    action_path = os.path.join(project_path, 'UpToDate.lbaction')

    update_bundle_version(action_path, version)
    sign_bundle(action_path)

    archive_path = archive_bundle(action_path, version)
    html_url = upload_release('nriley/LBUpToDate', version, archive_path, github_access_token)
